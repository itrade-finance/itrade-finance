...
class Debt extends Component {
  ...
  constructor() {
    ...
    this.state = {
      ...
      totalDebt: collateralOptions ? collateralOptions.reduce((accumulator, currentValue) => accumulator + currentValue.debt, 0) : 0,
      totalInterest: collateralOptions ? collateralOptions.reduce((accumulator, currentValue) => accumulator + currentValue.interest, 0) : 0,
      totalPrincipal: collateralOptions ? collateralOptions.reduce((accumulator, currentValue) => accumulator + currentValue.principal, 0) : 0
    }
  ...
  debtReturned = () => {
    ...
    this.setState({
      collateralOptions: collateralOptions,
      totalDebt: collateralOptions.reduce((accumulator, currentValue) => accumulator + currentValue.debt, 0),
      totalInterest: collateralOptions.reduce((accumulator, currentValue) => accumulator + currentValue.interest, 0),
      totalPrincipal: collateralOptions.reduce((accumulator, currentValue) => accumulator + currentValue.principal, 0)
     })
  };
        ...
        { !account.address &&
          <div className={ classes.notConnectedContainer }>
            ...
            <div className={ classes.introCenter }>
              <Typography variant='h2'>{ t('Debt.Intro') }</Typography>
            </div>
            <div className={ classes.connectContainer }>
              <Button
                ...
                >
                <Typography className={ classes.buttonText } variant={ 'h5'}>{ t('Debt.Connect') }</Typography>
              </Button>
            </div>
          </div>
        }
          ...
          <div className={ classes.card }>
            ...
            <div className={ classes.totalsContainer }>
              <Card className={ classes.totalCard }>
                <Typography variant={'h1'} className={ classes.totalAmount }>$ { totalPrincipal ? totalPrincipal.toFixed(4) : '0.0000' }</Typography>
                <Typography variant={'h3'} className={ classes.totalTitle }>Total Principal</Typography>
              </Card>
  ...
  renderAssetBlock = () => {
    const { collateralOptions, expanded } = this.state
    const { classes, t } = this.props
    const width = window.innerWidth

    return collateralOptions.map((asset) => {
      return (
          ...
          >
            <div className={ classes.assetSummary }>
              ...
              <div className={classes.heading}>
                <Typography variant={ 'h3' }>{ (asset.principal).toFixed(4) + ' ' + asset.symbol }</Typography>
                <Typography variant={ 'h5' }>{ t('Debt.Principal') }</Typography>
              </div>
              <div className={classes.heading}>
                <Typography variant={ 'h3' }>{(asset.debt).toFixed(4)+' ' + asset.symbol}</Typography>
                <Typography variant={ 'h5' }>{ t('Debt.Debt') }</Typography>
              </div>
              <div className={classes.heading}>
                <Typography variant={ 'h3' }>{(asset.position).toFixed(4)+' ' + asset.symbol}</Typography>
                <Typography variant={ 'h5' }>{ t('Debt.Position') }</Typography>
              </div>
            </div>
          </ExpansionPanelSummary>
          <ExpansionPanelDetails>
...
export default withNamespaces()(withRouter(withStyles(styles)(Debt)));
